    ...
    if result == "OK":
        ...
        for i in range(0,numEmails):
            ...
            try:
                ...
                sys.stdout.write("\r%d emails fetched..." % (i+1))
                sys.stdout.flush()
            except:
                pass
    print "\n"
...
Returns a dictionary with sender email addresses mapped to an associated address.
'''
...
def getDirections(conn, start, dest):
    while True:
        try:
            ...
            directions_result = conn.directions(start, dest, departure_time=now)
        except:
